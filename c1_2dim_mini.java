...
//	次の時間での温度の計算
		for (k = 2; k <= my-1; k++)
		{
			for (j = 2; j <= mx-1; j++)
			{
				...
				uu[j*11+k]= u[j*11+k]+r1*(u[(j+1)*11+k]-2.0f*u[j*11+k]+u[(j-1)*11+k])
					+r2*(u[j*11+k+1]-2.0f*u[j*11+k]+u[j*11+k-1])+dt*q[j*11+k];
				//System.out.println(k+" "+j+" "+uu[j*11+k]);
			}
		}
