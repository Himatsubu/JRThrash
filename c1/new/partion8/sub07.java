...
public class sub07 extends Thread{
        final float[] u      = new float[16641];
	//final float[] q      = new float[16900];
	final float[] result = new float[16641];
	float dx,dy,dt,r1,r2;

        @JRThrashUnroll(unrollNum=6, loopVariableName="k",unrollType=JRThrashUnroll.copyLoopVar)
        public void run(){
		int k,j;
                dx=1f/128;
		dy=1f/128;
		...
		for(j=65;j<=128;j++){
			...
			+r2*(u[j*129+k+1]-2.0f*u[j*129+k]+u[j*129+k-1])+dt*100f*dx*(j-1)*dy*(k-95-1);			
